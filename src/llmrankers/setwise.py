...
def move_past_key_values_to_device(past_key_values, device):
    return tuple(
        tuple(t.to(device) for t in layer) for layer in past_key_values
    )
...
class SetwiseLlmRanker(LlmRanker):
    ...
    def __init__(self,
        ...
        self.total_doc_tokens = []
        self.flops = []
        self.time = []

    def compare(self, query: str, docs: List):
        self.total_compare.append(1) if self.num_permutation == 1 else self.num_permutation
        doc_tokens = []
        for d in docs:
            input_ids = self.tokenizer(d.text, return_tensors="pt").input_ids.to(self.device)
            doc_tokens.append(input_ids.shape[1])
        self.total_doc_tokens.append(doc_tokens)
        ...
        if self.scoring == 'generation':
            if self.config.model_type == 't5':
                ...
                if self.num_permutation == 1:
                    ...
                    self.total_prompt_tokens.append(input_ids.shape[1])
                    ...
                    # flops, macs, params = calculate_flops(model=self.llm,
                    #                 kwargs = inputs,
                    ...
                    self.total_completion_tokens.append(output_ids.shape[0])
                ...
                self.total_prompt_tokens.append(inputs['input_ids'].shape[1])
                ...
                tic = time.time()
                output_ids = self.llm.generate(**inputs)[0]
                self.time.append(time.time() - tic)

                output_ids = output_ids[inputs['input_ids'].shape[1]:]
                self.total_completion_tokens.append(len(output_ids))
