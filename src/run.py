...
def main(args):
    ...
    dl20queries = pd.read_csv('dl20queries.txt', header=None, sep=' ')[0].unique()
    ...
    with open(args.run.run_path, 'r') as f:
        ...
        for line in tqdm(f):
            ...
            if args.run.ir_dataset_name[-2] == '2':
                if int(qid) in dl20queries:
                    if qid != current_qid:
    ...
    for qid, query, ranking in tqdm(first_stage_rankings):
        ...
        total_comparisons.append(ranker.total_compare)
        total_prompt_tokens.append(ranker.total_prompt_tokens)
        total_doc_tokens.append(ranker.total_doc_tokens)
        total_completion_tokens.append(ranker.total_completion_tokens)
        total_flops.append(ranker.flops)
        total_time.append(ranker.time)

    pd.DataFrame({
    'comparisons': total_comparisons,
    'input_length': total_prompt_tokens,
    'docs_length': total_doc_tokens,
    'output_length': total_completion_tokens,
    'flops': total_flops,
    'time': total_time,
}).to_csv('results/'+ args.run.model_name_or_path + '_' + args.run.ir_dataset_name[-2] + '_' + str(args.setwise.num_child) +'.csv', index=False)
